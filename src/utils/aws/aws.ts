...
export async function getFileFromDisk(fileName: string): Promise<Buffer> {
    ...
    res = await fsPromises.readFile(fileName)
        .then(content => {
            return content
        })
        .catch(err => {
...
export const uploadFilesToS3 = async (modelFile: any, coverPhotoFile: any, imageKey: string, modelkey: string): Promise<UploadResult> => {
    try {
        const s3 = new AWS.S3({
            accessKeyId: process.env.AWS_ACCESS_KEY_ID,
            secretAccessKey: process.env.AWS_SECRET_ACCESS_KEY,
        ...
        const coverPhotoUploadResult = await s3.upload(coverPhotoUploadParams).promise();
...
export const UploadEvidenceToS3 = async (imageFile: any, evidenceKey: string): Promise<any> => {
    try {
        const s3 = new AWS.S3({
            accessKeyId: process.env.AWS_ACCESS_KEY_ID,
            secretAccessKey: process.env.AWS_SECRET_ACCESS_KEY,
...
export const UploadGtag = async (imageFile: any, evidenceKey: string): Promise<any> => {
    try {
        const s3 = new AWS.S3({
            accessKeyId: process.env.AWS_ACCESS_KEY_ID,
            secretAccessKey: process.env.AWS_SECRET_ACCESS_KEY,
...
export const UploadSampleToS3 = async (imageFile: any, imageKey: string): Promise<any> => {
    try {
        const s3 = new AWS.S3({
            accessKeyId: process.env.AWS_ACCESS_KEY_ID,
            secretAccessKey: process.env.AWS_SECRET_ACCESS_KEY,
...
export const UploadUserToS3 = async (imageFile: any, imageKey: string): Promise<any> => {
    try {
 ...
 */
export function extractAWSKeyFromCoverPhotoUrl(coverPhotoUrl: string): string | null {
    // Split the URL by '/'

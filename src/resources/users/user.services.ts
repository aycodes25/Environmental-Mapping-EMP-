		...
		if (user.password) {
			...
			if (!isPasswordValid) {
				...
				if (!user.twoFactorAuth.enabled) {
					...
					const user_without_password = await UserSchema.findOne({
						email: email,
					}).populate("locations");
			...
			throw new Error(
				"Please contact the administrator and error just occurred"
			);
		}
	} catch (error: any) {
...
export const signUpTagger = async (
	...
	try {
		...
		const usernameAlreadyExists = await UserSchema.findOne({
			username: username,
		});
...
export const getUsers = async (userId: any): Promise<any> => {
	try {
		...
		if (user.role === RoleType.superAdmin) {
			...
			users = await userModel
				.find({ role: { $ne: RoleType.superAdmin } })
				.sort({ createdAt: -1 });
		} else {
...
export const userUpdate = async (
	...
	try {
		...
		if (userData.username && user?.username !== userData.username) {
			let userWithUserName = await userModel.findOne({
	...
	try {
		...
		if (user.twoFactorAuth.secret) {
			...
			if (!user.twoFactorAuth.enabled && verified) {
				...
				return {
					status: "success",
					data: { user: new_without_secret, accessToken: token },
...
export const recentTaggers = async (): Promise<any> => {
	try {
		const recentTaggers = await userModel
			.find({ role: "tagger" })
			.sort({ createdAt: -1 })
			.limit(5);
...
export const recentReviewers = async (): Promise<any> => {
	try {
		const recentReviewers = await userModel
			.find({ role: "reviewer" })
			.sort({ createdAt: -1 })
			.limit(5);
...
export async function updatePasswordRelativity(
	userId: any,
	newPassword: string
): Promise<any> {
...
export async function verifyEmailRelativity(userId: any): Promise<any> {
	...
	await userModel.findByIdAndUpdate(userId, { isVerified: isEmailVerified });
}
...
export async function validateUser(
	...
	try {
		...
		if (
			...
			const user_without_password = await userModel.findOne({
				username: username,
			});
...
export async function register(createUserDto: any): Promise<any> {
	const existingUser = await findOneByUsername(createUserDto.username);
	if (existingUser) {
...
export async function verifyEmail(token: string): Promise<any> {
	try {
		const secretKey =
			process.env.EMAIL_VERIFICATION_SECRET || "default-verify-email-secret";

...
export class UserController {
    async signUpAdmin(req: AuthUserRequest, res: Response, next: NextFunction) {
        ...
        try {
            const data = await userService.signUpAdmin(email, password, username, photoFile, fullname)
            res.json({
                status: "success",
                data,
    ...
    async signUpTagger(req: AuthUserRequest, res: Response, next: NextFunction) {
        ...
        try {
            if (files) {
                ...
                data = await userService.signUpTagger(email, password, username, role, image, fullname, location)
    ...
    async updatePassword(req: AuthUserRequest, res: Response, next: NextFunction) {
        const { email, password } = req.body
        if (!email || !password) {
            res.status(401).json({ status: "error", message: 'missing credentials' })
        }
    ...
    async getAUser(req: AuthUserRequest, res: Response, next: NextFunction) {
        try {
            ...
            if (!id) {
                res.status(400).send({ status: "error", message: 'No user id is parsed' })
            }
    ...
    async twoFA(req: AuthUserRequest, res: Response) {
        try {
            ...
            res.status(200).json({ message: 'show user qr code', status: "success", secret: secret });
        } catch (error) {
            res.status(500).json({ message: 'Please try again an error occurred', status: "error" });
        }
    };
    ...
    async resetPassword(req: AuthUserRequest, res: Response) {
        try {
            ...
            res.status(200).json({ message: 'Please check your email for the reset link', status: "success" })

        } catch (error) {
            res.status(500).json({ message: 'try again failed to send reset link', status: "error" });
        }
    };
    ...
    async twoFAverifyToken(req: AuthUserRequest, res: Response) {
        try {
            ...
            res.status(500).json({ message: 'Failed to  verification token', status: 'error' });
        }
    };
    ...
    async resetPasswordVerifyToken(req: AuthUserRequest, res: Response) {
        try {
            ...
            console.log(error)
            res.status(500).json({ message: "Failed to verify token", status: 'error' });
        }
    }
    ...
    async dashboard(req: AuthUserRequest, res: Response) {
        ...
        try {
            ...
            const userLocation = user?.locations?.valueOf();
            ...
            const matchStage = userRole === 'superAdmin' ? { type: 'sampling' } : { type: 'sampling', 'modelInfo.location': userLocation, 'modelInfo.delete': false };

            // Get total tags this month grouped by sample
            const tagsThisMonth = await tagsModel.aggregate([
                { $match: { createdAt: { $gte: startOfMonth, $lte: endOfMonth }, ...matchStage } },
                {
                    $lookup: {
            ...
            const tagsLastMonth = await tagsModel.aggregate([
                { $match: { createdAt: { $gte: startOfLastMonth, $lte: endOfLastMonth }, ...matchStage } },
                {
                    $lookup: {
                        from: 'models', // The collection name for Model model
                        localField: 'model',
            ...
            const totalTagsThisMonth = await tagsModel.countDocuments({ createdAt: { $gte: startOfMonth, $lte: endOfMonth }, type: 'sampling', ...(userRole !== 'superAdmin' && { 'model.location': userLocation, 'model.delete': false }) });
            const positiveTagsThisMonth = await tagsModel.countDocuments({ createdAt: { $gte: startOfMonth, $lte: endOfMonth }, presence: 'positive', type: 'sampling', ...(userRole !== 'superAdmin' && { 'model.location': userLocation, 'model.delete': false }) });
            const positivityRateThisMonth = totalTagsThisMonth > 0 ? (positiveTagsThisMonth / totalTagsThisMonth) * 100 : 0;

            // Get total tags year-to-date grouped by sample
            const tagsYearToDate = await tagsModel.aggregate([
                { $match: { createdAt: { $gte: startOfYear, $lte: endOfYear }, ...matchStage } },
                {
                    $lookup: {
                        from: 'models', // The collection name for Model model
                        localField: 'model',
            ...
            const totalTagsYearToDate = await tagsModel.countDocuments({ createdAt: { $gte: startOfYear, $lte: endOfYear }, type: 'sampling', ...(userRole !== 'superAdmin' && { 'model.location': userLocation, 'model.delete': false }) });
            const positiveTagsYearToDate = await tagsModel.countDocuments({ createdAt: { $gte: startOfYear, $lte: endOfYear }, presence: 'positive', type: 'sampling', ...(userRole !== 'superAdmin' && { 'model.location': userLocation, 'model.delete': false }) });
            const positivityRateYearToDate = totalTagsYearToDate > 0 ? (positiveTagsYearToDate / totalTagsYearToDate) * 100 : 0;

            // Get positivity rate per month year-to-date
            const positivityRatePerMonthYearToDate = await tagsModel.aggregate([
                { $match: { createdAt: { $gte: startOfYear, $lte: endOfYear }, presence: { $in: ['positive', 'negative'] }, ...matchStage } },
                {
                    $lookup: {
                        from: 'models', // The collection name for Model model
                        localField: 'model',
                ...
                { $unwind: '$modelInfo' },
                {
                    $group: {
                        ...
                        total: { $sum: 1 },
                        positive: { $sum: { $cond: [{ $eq: ["$presence", "positive"] }, 1, 0] } }
                    }
                },
                {
                    $project: {
                        ...
                        year: "$_id.year",
                        month: "$_id.month",
                        positivityRate: { $cond: [{ $eq: ["$total", 0] }, 0, { $divide: ["$positive", "$total"] }] },
                        totalTags: "$total",
            ...
            // const recentTaggers = await userService.recentTaggers();
            ...
            const TotalTagsBySampleAndMonth = await TagsServices.getTotalTagsBySampleAndMonth(userId)
            const TotalIncidentsByMonth = await TagsServices.taggedIncidentbyMonth(userId)
            const TotalIncidentsByDay = await TagsServices.taggedIncidentbyDay(userId)
            const TotalTagsBySampleAndDay = await TagsServices.getTotalTagsBySampleAndDay(userId)
            // const totalInWeek = await modelService.weekModels();
            const recentlyViewedModels = await modelService.getRecentlyViewedModelsAndUsers()
            const modelsInEachLocation = await modelService.modelsEachLocation(userId);
            const recentlyTaggeddModels = await modelService.recentlyTaggeddModels(userId)
            const modelsByDays = await modelService.getTotalModelsPerDayOfWeek(userId)
            const modelsByMonth = await modelService.getTotalModelsPerMonth(userId)
            //const taggedSamplesByday = await TagsServices.taggedSamplesByday()
            res.status(200).json({
                tagsThisMonth,
                tagsLastMonth,
                totalTagsThisMonth,
    ...
    async dashboardByLocation_old(req: AuthUserRequest, res: Response) {
        ...
        try {
            ...
            const matchStage = { type: 'sampling', 'modelInfo.location': locationId, 'modelInfo.delete': false };
            ...
            // Get positivity rate this month
            const totalTagsThisMonth = await tagsModel.countDocuments({ createdAt: { $gte: startOfMonth, $lte: endOfMonth }, type: 'sampling', 'model.location': locationId });
            const positiveTagsThisMonth = await tagsModel.countDocuments({ createdAt: { $gte: startOfMonth, $lte: endOfMonth }, presence: 'positive', type: 'sampling', 'model.location': locationId });
            const positivityRateThisMonth = totalTagsThisMonth > 0 ? (positiveTagsThisMonth / totalTagsThisMonth) * 100 : 0;
            ...
            // Get positivity rate year-to-date
            const totalTagsYearToDate = await tagsModel.countDocuments({ createdAt: { $gte: startOfYear, $lte: endOfYear }, type: 'sampling', 'model.location': locationId, 'model.delete': false });
            const positiveTagsYearToDate = await tagsModel.countDocuments({ createdAt: { $gte: startOfYear, $lte: endOfYear }, presence: 'positive', type: 'sampling', 'model.location': locationId, 'model.delete': false });
            const positivityRateYearToDate = totalTagsYearToDate > 0 ? (positiveTagsYearToDate / totalTagsYearToDate) * 100 : 0;
            ...
            const deletedModels = await modelModel.countDocuments({ location: locationId, delete: true });

            // Query for recent models in the location (e.g., created in the last 7 days)
            const recentModels = await modelModel.find({
                delete: false,
                location: locationId,
            ...
            const totalReviewers = await userModel.countDocuments({ location: locationId, role: 'reviewer' });

            // Query for today's models in the location
            const todaysModels = await modelModel.find({
                location: locationId,
                createdAt: { $gte: new Date(now.setHours(0, 0, 0, 0)), $lt: new Date(now.setHours(23, 59, 59, 999)) }
            }).sort({ createdAt: -1 });

            // Query for total taggers in the location
            const totalTaggers = await userModel.countDocuments({ location: locationId, role: 'tagger' });

            // Query for total tags by sample and month
            const TotalTagsBySampleAndMonth = await tagsModel.aggregate([
                { $match: { createdAt: { $gte: startOfYear, $lte: endOfYear }, 'model.location': locationId, 'model.delete': false } },
                {
                    $group: {
            ...
            const TotalIncidentsByMonth = await tagsModel.aggregate([
                { $match: { createdAt: { $gte: startOfYear, $lte: endOfYear }, 'model.location': locationId, 'model.delete': false, type: 'incident' } },
                {
                    $group: {
            ...
            const TotalIncidentsByDay = await tagsModel.aggregate([
                { $match: { createdAt: { $gte: startOfMonth, $lte: endOfMonth }, 'model.location': locationId, 'model.delete': false, type: 'incident' } },
                {
                    $group: {
            ...
            const TotalTagsBySampleAndDay = await tagsModel.aggregate([
                { $match: { createdAt: { $gte: startOfMonth, $lte: endOfMonth }, 'model.location': locationId, 'model.delete': false } },
                {
                    $group: {
    ...
    async dashboardByLocation(req: AuthUserRequest, res: Response) {
        try {
            ...
            const [tagsMonth, positivityRateMonth, tagsYearToDate, positivityRatePerMonthYearToDate, totalModels, deletedModels, recentModels, totalReviewers, todaysModels, totalTaggers, totalTagsBySampleAndMonth, totalIncidentsByMonth, totalIncidentsByDay, totalTagsBySampleAndDay, modelsInEachLocation, recentlyTaggedModels, modelsByDays, tagsLastMonth, totalTagsThisMonth, positiveTagsThisMonth, modelsByMonth, positivityRateYearToDate] = await Promise.all([
                ...
                getPositivityRatePerMonthYearToDate(startOfYear, endOfYear, locationId),
...
async function getTagsByPeriod(startDate: Date, endDate: Date, locationId: string) {
    return tagsModel.aggregate([
        { $match: { createdAt: { $gte: startDate, $lte: endDate }, type: 'sampling', 'modelInfo.location': locationId, 'modelInfo.delete': false } },
        { $lookup: { from: 'models', localField: 'model', foreignField: '_id', as: 'modelInfo' } },
        { $unwind: '$modelInfo' },
        { $lookup: { from: 'samples', localField: 'sample', foreignField: '_id', as: 'sampleInfo' } },
        { $unwind: '$sampleInfo' },
        { $group: { _id: "$sampleInfo.name", count: { $sum: 1 } } }
    ]);
}

async function getPositivityRate(startDate: Date, endDate: Date, locationId: string) {
    const totalTags = await tagsModel.countDocuments({ createdAt: { $gte: startDate, $lte: endDate }, type: 'sampling', 'model.location': locationId });
    const positiveTags = await tagsModel.countDocuments({ createdAt: { $gte: startDate, $lte: endDate }, presence: 'positive', type: 'sampling', 'model.location': locationId });
    return totalTags > 0 ? (positiveTags / totalTags) * 100 : 0;
}

async function getTagsYearToDate(startOfYear: Date, endOfYear: Date, locationId: string) {
    return tagsModel.aggregate([
        { $match: { createdAt: { $gte: startOfYear, $lte: endOfYear }, type: 'sampling', 'model.location': locationId, 'model.delete': false } },
        { $lookup: { from: 'models', localField: 'model', foreignField: '_id', as: 'modelInfo' } },
        { $unwind: '$modelInfo' },
        { $lookup: { from: 'samples', localField: 'sample', foreignField: '_id', as: 'sampleInfo' } },
        { $unwind: '$sampleInfo' },
...
async function getTagsLastMonth(startOfLastMonth: Date, endOfLastMonth: Date, locationId: string) {
    return tagsModel.aggregate([
        { $match: { createdAt: { $gte: startOfLastMonth, $lte: endOfLastMonth }, type: 'sampling', 'model.location': locationId, 'model.delete': false } },
        { $group: { _id: null, count: { $sum: 1 } } }
    ]);
}

async function getTotalTagsThisMonth(startOfMonth: Date, endOfMonth: Date, locationId: string) {
    return tagsModel.countDocuments({ createdAt: { $gte: startOfMonth, $lte: endOfMonth }, type: 'sampling', 'model.location': locationId, 'model.delete': false });
}

async function getPositiveTagsThisMonth(startOfMonth: Date, endOfMonth: Date, locationId: string) {
    return tagsModel.countDocuments({ createdAt: { $gte: startOfMonth, $lte: endOfMonth }, presence: 'positive', type: 'sampling', 'model.location': locationId, 'model.delete': false });
}

async function getModelsByMonth(startOfYear: Date, endOfYear: Date, locationId: string) {
    return modelModel.aggregate([
        { $match: { location: locationId, delete: false, createdAt: { $gte: startOfYear, $lte: endOfYear } } },
...
async function getRecentModels(locationId: string, now: Date) {
    return modelModel.find({ location: locationId, delete: false, createdAt: { $gte: new Date(now.setDate(now.getDate() - 7)) } }).sort({ createdAt: -1 });
}
...
async function getTodaysModels(locationId: string, now: Date) {
    return modelModel.find({ location: locationId, createdAt: { $gte: new Date(now.setHours(0, 0, 0, 0)), $lt: new Date(now.setHours(23, 59, 59, 999)) } }).sort({ createdAt: -1 });
}
...
async function getTotalTagsBySampleAndMonth(startOfYear: Date, endOfYear: Date, locationId: string) {
    return tagsModel.aggregate([
        { $match: { createdAt: { $gte: startOfYear, $lte: endOfYear }, 'model.location': locationId, 'model.delete': false } },
        { $group: { _id: { month: { $month: "$createdAt" }, sample: "$sample" }, count: { $sum: 1 } } },
        { $sort: { '_id.month': 1 } }
    ]);
}

async function getTotalIncidentsByMonth(startOfYear: Date, endOfYear: Date, locationId: string) {
    return tagsModel.aggregate([
        { $match: { createdAt: { $gte: startOfYear, $lte: endOfYear }, 'model.location': locationId, 'model.delete': false, type: 'incident' } },
        { $group: { _id: { month: { $month: "$createdAt" } }, count: { $sum: 1 } } },
        { $sort: { '_id.month': 1 } }
...
async function getTotalIncidentsByDay(startOfMonth: Date, endOfMonth: Date, locationId: string) {
    return tagsModel.aggregate([
        { $match: { createdAt: { $gte: startOfMonth, $lte: endOfMonth }, 'model.location': locationId, 'model.delete': false, type: 'incident' } },
...
async function getTotalTagsBySampleAndDay(startOfMonth: Date, endOfMonth: Date, locationId: string) {
    return tagsModel.aggregate([
        { $match: { createdAt: { $gte: startOfMonth, $lte: endOfMonth }, 'model.location': locationId, 'model.delete': false } },
        { $group: { _id: { day: { $dayOfMonth: "$createdAt" }, sample: "$sample" }, count: { $sum: 1 } } },
        { $sort: { '_id.day': 1 } }
    ]);
}
...
async function getPositivityRateYearToDate(startOfYear: Date, endOfYear: Date, locationId: string): Promise<number> {
